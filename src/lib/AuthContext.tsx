...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchUserRole = async (userId?: string, userEmail?: string) => {
    ...
    if (userEmail && userEmail.toLowerCase() === 'banhbaomangcut@gmail.com') {
      setRole('admin');
      return;
    }
  ...
  const mockSignIn = (isAdmin: boolean = false) => {
    const mockUser = { id: isAdmin ? 'mock-admin-123' : 'mock-user-123', email: isAdmin ? 'banhbaomangcut@gmail.com' : 'demo@courtkings.vn' } as User;
